...
def ResetEncoders():
    global EncoderCountsLeft,EncoderCountsLeftPrev,EncoderCountsRight,EncodeCountsRightPrev
    EncoderCountsRight = 0
    EncoderCountsRightPrev = 0
    EncoderCountsLeft = 0
    EncoderCountsLeftPrev = 0
    MPHCounter = 0
    MPHTimer = time.ticks_us()
...
#user code goes here
    #Get MPH
    if MPHCounter == 50:
        Elapsed = time.ticks_us() - MPHTimer
        print(Elapsed)
        RightRPM = ((EncoderCountsRight-EncoderCountsRightPrev)/((Elapsed/1000000)*36))*60
        MPHRight = (RightRPM*TireDiameter*3.14*60)/63360
        LeftRPM = ((EncoderCountsLeft-EncoderCountsLeftPrev)/(.5*36))*60
        MPHLeft = (LeftRPM*TireDiameter*3.14*60)/63360
        MPHTimer = time.ticks_us()
...
#Start Accelerate and decellerate Run
    if Cycles > 0:
        ...
        # Wait until distance is reached then go into reverse    
            ...
            #print('PS1')
            if ((DistanceLeft >= TargetDistance) or (DistanceRight >= TargetDistance)):
                #Reverse()
                #Brake algorithm here
                TargetVelocity = 0 # set velocity to 0
                if ((SignalToMotorLeft < 1505000) or (SignalToMotorRight < 1505000)):
                    SignalToMotorLeft = 1480000
                    SignalToMotorRight = 1480000
        ...
        #wait until reverse distance is reached
            ...
            if ((DistanceLeft >= TargetDistance) or (DistanceRight >= TargetDistance)):
                TargetVelocity = 0
                if ((SignalToMotorLeft > 1480000) or (SignalToMotorRight > 1480000)):
                    SignalToMotorLeft = 1500000
                    SignalToMotorRight = 1500000
    ...
    #PID start
        ...
        if ReverseMode:
            errorRight = (TargetVelocity + MPHRight)*2
            #print('ErrorRight {}'.format(errorRight))
            errorLeft = (TargetVelocity + MPHLeft)*2
        ...
        if TargetVelocity == 0:
            if ReverseMode:
                SignalToMotorLeft += int(errorLeft*200 + errorLeftPrev*24)
                SignalToMotorRight += int(errorRight*200 + errorRightPrev*27)
            else :
    ...
    #PID end
        ...
        if (((SignalToMotorRight > 1420000)and(SignalToMotorRight < 1570000)) and ((SignalToMotorLeft > 1420000)and(SignalToMotorLeft < 1570000))):
    ...
    """
    if printtimer == 10:
        print('Encoders L: {} R: {} PWML : {} PWMR {}'.format(EncoderCountsLeft, EncoderCountsRight,SignalToMotorLeft,SignalToMotorRight))
        printtimer = 0
    printtimer += 1
    """
#end code
